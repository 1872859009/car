...
volatile uint32_t receive_counter = 0;	// 定义GPS接收计数器
char GPS_RxData[100];			//定义用于接收串口数据的变量
char latitude[20];    			// 存储纬度的字符串
char longitude[20];   			// 存储经度的字符串
...
void GPS(void)                           //GPS数据获取函数
{
	 uint8_t GPS_dataflag;			//接收GPS数据标志位
if (Serial3_RxFlag == 1)		//如果接收到数据包
		{
			strcpy(GPS_RxData, Serial3_RxPacket);//复制接收数组到GPS_RxData
			receive_counter = 0;	// 复位接收计数器
			Serial3_RxFlag = 0;     //处理完成后，需要将接收数据包标志位清零，否则将无法接收后续数据包
		}
		/***********解析 GPS_RxData 中的数据，提取纬度和经度信息*******/
		
		 /*如果接收到的数据包含 'V'，则显示 "Not positioned"，否则开始解析数据*/
		if (strstr(GPS_RxData, "V") != NULL) 
		{
//			OLED_ShowString(1, 1, "Not positioned");   // 显示未定位成功信息
//			OLED_ShowString(2, 1, "              ");   // 显示纬度信息
//			OLED_ShowString(3, 1, "              ");   // 显示纬度信息


		}
		
		else if (strstr(GPS_RxData, "A") != NULL) 
		{
			char* token;
			token = strtok(GPS_RxData, ",");
			int count = 0;
			while (token != NULL) 
			{
				if (count == 3) 
				{
					strcpy(latitude, token);// 第3个字段是纬度信息
					strcat(latitude, ",N"); // 加上 'N'
				} 
				else if (count == 5)
				{
					strcpy(longitude, token);// 第5个字段是经度信息
					strcat(longitude, ",E"); // 加上 'E'
				}
				token = strtok(NULL, ",");
				count++;
				if(!GPS_dataflag)
...
//				OLED_ShowString(1, 1, "              ");   // 
//				OLED_ShowString(2, 1, latitude);   // 显示纬度信息
//				OLED_ShowString(3, 1, longitude);  // 显示经度信息
